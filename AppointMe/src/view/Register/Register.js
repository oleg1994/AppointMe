      ...
      fetch('http://localhost:4000/newUser', {
        ...
        .then(response => {
          if (response.success) {
            dispatchappState({
              type: 'popUp',
              payload: { status: true,message:'Thanks for registering, you can log-in now',url:'/' }  //payload
            })
          }
          if (response.check) {
  ...
  return (
    <div>
      ...
      <form className='registerWrapper' onSubmit={register}>
        ...
        <div className='inputPos'><input className='OrangeButton' type='submit' value='register' required></input></div>
      </form>
    </div>
  );
}
